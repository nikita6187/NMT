...
python3 /u/makarov/returnn-merge/rnn.py ${CONFIG} ++load_epoch ${EPOCH} ++device gpu --task search ++search_data config:newstest${YEAR} ++beam_size ${BEAM_SIZE} ++need_data False ++max_seq_length 0 ++search_output_file ${OUTPUT_FOLDER}scoring_${YEAR}_beam${BEAM_SIZE}.bpe ++batch_size 600 ++log ${OUTPUT_FOLDER}search_log_${YEAR}_beam${BEAM_SIZE}.log ++max_seqs ${MAX_SEQS}
deactivate
